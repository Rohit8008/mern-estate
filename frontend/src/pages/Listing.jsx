...
function ShareLocationButton({ lat, lng, name }) {
  ...
  return (
    ...
    >
      {copied ? (
        <>
          <HiOutlineCheck className='w-3.5 h-3.5' aria-hidden='true' />{t('listing.linkCopied')}</>
      ) : (
        <>
          <HiOutlineShare className='w-3.5 h-3.5' aria-hidden='true' />{t('listing.shareLocation')}</>
      )}
    </button>
  );
}
    ...
    <main>
      ...
      {listing && !loading && !error && (
        <div>
          ...
          >
            <HiOutlineShare className='text-slate-600 w-5 h-5' aria-hidden='true' />
          </button>
          {copied && (
            <p role='status' className='fixed top-36 right-6 z-10 rounded-md bg-slate-900 text-white text-xs px-2 py-1 shadow'>{t('listing.linkCopied2')}</p>
          )}
          {isStaff && (
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  {locationLine && (
                    <p className='flex items-center gap-2 text-slate-600 text-sm'>
                      <HiOutlineLocationMarker className='text-slate-400 w-4 h-4' aria-hidden='true' />
                      {locationLine}
                    </p>
                  )}
                              ...
                              <div className='space-y-1'>
                                {owner.email && (
                                  <p className='text-sm text-slate-600 flex items-center gap-2'>
                                    <svg className='w-4 h-4 text-slate-400' aria-hidden='true' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
